    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                else if (result.error.code === 'auth/invalid-email') friendlyMessage = 'Email không hợp lệ.';
                else if (result.error.code === 'auth/weak-password') friendlyMessage = 'Mật khẩu quá yếu (ít nhất 6 ký tự).';

                Alert.alert("Đăng ký thất bại", friendlyMessage);
            }
        } catch (error) {
            console.log("Register Error:", error);
            Alert.alert("Lỗi", "Lỗi không mong muốn.");
        } finally {
            setLoading(false);
        }
    };
